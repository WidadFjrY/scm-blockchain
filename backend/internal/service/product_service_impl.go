...
func (serv *ProductServiceImpl) Create(ctx context.Context, request web.ProductCreateRequest, role string) web.ProductCreateResponse {
	...
	err := serv.DB.Transaction(func(tx *gorm.DB) error {
		for _, productModel := range serv.ProductRepo.GetAll(ctx, tx) {
			product := web.ProductResponse{
				ID:            productModel.ID,
				DistributorID: productModel.DistributorID,
